/**
 * Copyright 2016-18 Willem Eradus and SODAQ
...
UBlox::UBlox(TwoWire &aWire, uint8_t address) : Wire_(aWire)
{
    address_ = address;
    //
    Id_ = AckedId_ = plLength_ = 0;
...
void UBlox::dispatchMessage(int id)
{
    ...
    if (id > 0)
    { // Valid and not equal to ACK or NAK
        switch (id)
        {
        case UBX_ACK_ACK:
            db_printf("ACK received\n");
            break;
        case UBX_ACK_NAK:
            db_printf("NAK received\n");
            break;
        case UBX_NAV_PVT:
            if (funcNavPvt)
            {
            ...
            {
                db_printf("No function Nav Pvt defined.\n");
            }
            break;
        default:
            print_buffer();
...
void UBlox::CfgMsg(uint16_t Msg, uint8_t rate)
{
    ...
    buffer[0] = 0x06; // Class
    buffer[1] = 0x01; // ID
    buffer[2] = 0x03; // length - first byte
